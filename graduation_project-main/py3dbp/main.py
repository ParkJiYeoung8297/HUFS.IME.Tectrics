        ...
        for i in range(0, len(rotate)):
            ...
            if fit:
                ...
                if self.fix_point == True :
                    ...
                    item.position = [x,y,z]
                    item.width, item.height, item.depth = dimension[0], dimension[1], dimension[2]
                if fit :
...
class Packer:
    ...
    def pack2Bin(self, bin, item,fix_point,check_stable,support_surface_ratio):
        ...
        if bin.corner != 0 and not bin.items:
            ...
            return
        axex_priority = [Axis.HEIGHT, Axis.DEPTH, Axis.WIDTH]
        for axis in axex_priority:
    ...
    def pack(self, bigger_first=False,distribute_items=True,fix_point=True,check_stable=True,support_surface_ratio=0.75,binding=[]):
        '''pack master func '''
        # set decimals
        ...
        # 리스트를 JSON 파일로 저장
                if items_data:
                    try:
                        with open('packed_items.json', 'w') as json_file:
    ...
    # 각 bin에 있는 아이템들의 정보를 JSON 파일로 저장
# def save_items_to_json(packer, file_path):
#     items_data = []

#     # 모든 bins의 아이템들을 순회
#     for bin in packer.bins:
#         for item in bin.items:
#             # 각 아이템의 정보를 딕셔너리로 변환하여 리스트에 추가
#             item_data = {
#                 "partno": item.partno,
#                 "width": item.width,
#                 "height": item.height,
...
# # 파일을 저장할 경로
# json_file_path = 'packed_items.json'

# # JSON 파일 저장
# save_items_to_json(Packer, json_file_path)
        ...
        # pack 메소드가 실행된 후, 모든 bins에 대한 항목 정보를 json 파일로 저장하는 함수
        # def save_items_to_json(packer, file_path):
        #     items_data = []
        ...
        #     for bin in packer.bins:
        #         for item in bin.items:
        #             # 각 항목의 정보를 딕셔너리 형태로 변환하여 리스트에 추가
        #             item_data = {
        #                 "partno": item.partno,
        #                 "width": item.width,
        #                 "height": item.height,
        ...
        # # 기존 코드의 실행 부분
        # # packer_box1.pack(...) 와 packer_box2.pack(...) 호출 이후에 이 함수를 호출합니다.
        # # JSON 파일을 저장하고자 하는 경로
        # json_file_path = '/Users/hwang-yechan/HUFS/Capstone/HUFS.IME.Tectrics/three.js-master/study/packed_items.json'

        # # packer_box1과 packer_box2에 대해 JSON 파일 저장
        # save_items_to_json(packer_box1, json_file_path)
        # save_items_to_json(packer_box2, json_file_path)
